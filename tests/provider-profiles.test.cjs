...
assert.deepEqual(providerBatchProfile('gemini'), { maxItems: 400, maxChars: 32000 });
...
assert.deepEqual(requestBatches(Array.from({ length: 401 }, (_, index) => ({ modId: index % 2 ? 'one' : 'two', source: 'short' })), 'gemini').map(batch => batch.length), [400, 1]);
